...
function Card({ title, image, desc, locate }) {
  return (
    <div className="rounded-3xl overflow-hidden flex flex-col xl:flex-row">
      <img src={image || '/bromo.jpg'} alt={title || 'Image'} className="w-96 h-full object-cover" />
      <div className=" bg-white px-4 py-4 rounded-2xl  flex flex-col items-center justify-center">
        <div className='w-[22rem] xl:w-[31rem]'>
        <h3 className="font-montserratsem font-extrabold text-[1.3rem] text-gray-800">{title || 'Pendakian Bukit Semar yang Memanjakan Mata'}</h3>
        <p className="font-montserratreg text-sm mt-2 text-gray-600 overflow-hidden text-ellipsis whitespace-normal h-12 line-clamp-2">
